...
if 'unique_transformed_cols' in st.session_state:
    ...
    with st.form('Create Lagged Variables'):
        variable_to_lag = st.selectbox('X-variable', [col for col in st.session_state['df_transformed'].columns])
...
if st.button('Implement Lag'):
    ...
    df_transformed = qu.lag_variables(df_transformed, st.session_state['lag'])
...
if st.button('Run Regression'):
    ...
    if 'df_transformed_lag' in st.session_state and 'selected_x_variables' and 'n_step_ahead' and 'start_date' in st.session_state:
        ...
        for window in tqdm(st.session_state['windows']):
            df_results, fitted_models, X_series, y_series = qu.rolling_regression_sklearn_advanced(df_regression, rolling_window=window, n_step_ahead=st.session_state['n_step_ahead'],
